...
class Topbar {
    ...
    dom : any
    userInfo : any                                                                          // 用户信息
    delCartDom: any                                                                         // 删除的购物车商品dom元素
    cartNum : any                                                                           // 购物车数量
    cartTime : any                                                                          // 购物车请求接口时间 ps: 限制其1s内最多请求一次
    delCartInfo: any                                                                         // 删除的购物车商品信息 ps: 仅保存1条数据
    areaInfo: any
    tpl: {
        areaTpl: any,
        nickNameTpl: any,
    ...
    constructor() {
        ...
        this.tpl = {
            areaTpl : require('../dot/area-tpl.dot'),                               // 所在地模板
            nickNameTpl: require('../dot/nick-name-tpl.dot'),                       //用户名模板
            sellerCenterTpl: require('../dot/seller-center-tpl.dot'),               // 卖家中心模板
            loginTpl: require('../dot/login-tpl.dot'),                              // 登录状态下登录弹窗模板
            noLoginTpl: require('../dot/no-login-tpl.dot'),                         // 未登录状态下登录弹窗模板
            cartNoneTpl:  require('../dot/cart-none-tpl.dot'),                      // 登录状态下购物车为空模板
            noLoginCartNoneTpl: require('../dot/no-login-cart-none-tpl.dot'),       // 未登录状态下购物车为空模板
            cartListTpl : require('../dot/cart-list-tpl.dot'),                      // 购物车列表模板
            navLoadTpl : require('../dot/nav-load-tpl.dot'),                        // load模板
    ...
    contactManagerConf (){
        var that = this;
        window.KFZ.tools.contactManager.on('msgNotice',function( unreadNum:any ){
            that.UnreadNumRender( unreadNum );
        },{
            from : '公共头部',
            description : '未读消息数量发生变化 -> 同步头部消息数量 contactManagerConf'
        });
        window.KFZ.tools.contactManager.on('areaNotice',function( area:any ){
     ...
     */
    delCartItem (delParam:any, cartDom:any){
        var that = this;
        $.ajax({
     ...
     */
    currentUrl (){
        let hash = window.location.hash.substr(1).replace(/\?.*/g, ''), href = window.location.href.replace(/#[\w-\/]*/gi, '').replace(/\?returnUrl=.*$/, '');
     ...
     * 更新 areaWin
     */
    updateArea ( area:any, activeDom:any ){
        area = area;
        const areaName = this.areaInfo[area];
        const $tag = activeDom || this.dom.areaWin.find('.text[code=' + area + ']');
        const flag = $tag.parent().hasClass('active');
        if(flag){
            return false;
        }
    ...
    areaInit (){
        var that = this,
            area = (this.userInfo && this.userInfo.area) || window.KFZ.tools.cookies('reciever_area');
        if(!area){
            area = '1000000000';
        }
        ...
        this.areaRender();
        this.dom.areaWin.find('.text').click(function(e:any){
            e.stopPropagation()
            // @ts-ignore
            var $tag = $(this),
                area = $tag.attr('code');
    ...
    loginWinRender (){
        ...
        that.userInfo.photo && window.KFZ.tools.loadImage(that.userInfo.photo,function( img:any ){
     ...
     * 通知模块通信管理器'delCart'
     */
    delCartNotice ( param:any, description:any ){
     ...
     */
    imNoticeConnectNotice ( description:any ){
     ...
     */
    revokeCartNotice ( description:any ){
     ...
     */
    userInfoLoadedNotice ( userInfo:any, description:any ){
     ...
     */
    updateAreaNotice ( area:any, description:any ){
